...
const Acheive = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://test.black-analysis-solutions.com/api/achieved');
  ...
  return (
    <div className='acheive'>
      ...
      <div >
        {data.map((e) => (
          <Article key={e.id} dateArticle={e.date} text={e.description} images={e.action} isList={true} />
        ))
        }
      </div>
    </div>
