...
export default function TheHeros() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('https://test.black-analysis-solutions.com/api/versions');
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (version) { // تحقق من أن النسخة ليست فارغة
                try {
                    const response = await axios.get(`https://test.black-analysis-solutions.com/api/internhero?version=${version}`);
                ...
                try {
                    const response = await axios.get(`https://test.black-analysis-solutions.com/api/internhero`);
                    setData2(response.data.data);
                    console.log(response.data.data);
                } catch (error) {
                    console.error('Error fetching data: ', error);
